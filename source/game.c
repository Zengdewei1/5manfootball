...
void game(int position1,int position2,int color1,int color2)
{
	...
	_team myteam;
	_team opteam;
	// myteam.player=(_player*)malloc(sizeof(_player)*4);
	// opteam.player=(_player*)malloc(sizeof(_player)*4);
	// if(myteam.player==NULL||opteam==NULL)
	// 	exit(0)
	myteam.name=Player;
...
void AttackExecute(_team *pmyteam,_team *popteam,_ball *pball)
{
	if(popteam->pnowstate==&popteam->Attack)//电脑进攻时
	{
		// setfillstyle(1,BLACK);
		// circle(10,10,10);
		if(popteam->control==0&&pball->timecount%15==0)//0号传球给1号
		{
		...
		if(popteam->control==1&&popteam->player[3].pnowstate==&popteam->player[3].Actioning&&pball->timecount%15==0)//1号传球给3号
		{
		...
		if(popteam->control==3&&distance(popteam->player[3].now_pos.x,popteam->player[3].now_pos.y,160.0,400.0)<10.0&&pball->timecount%15==0)//3号射门
		{
			...
			PlayerChangestate(pmyteam,popteam,&popteam->player[3],pball,&popteam->player[1].Wait);
...
{
	if(popteam->pnowstate==&popteam->Attack)//找到玩家队伍离球最近的球员，设为玩家控制的球员
	{
...
{
	Pos2d near_dir;
	if(pplayer->name==Player)//键盘操控+距离判断
	{
...
void DribbleEnter(_team *pmyteam,_team *popteam,_player *pplayer,_ball *pball)//进入带球状态，前往点位
{
...
void DribbleExecute(_team *pmyteam,_team *popteam,_player *pplayer,_ball *pball)
{
	...
	else//距离判断，到达点位后进入行动状态
	{
		...
		// if(pball->timecount%30==0)//另一方法，sin/cos,变正负
		// 	pplayer->velocity.y*=(-1);
	}
}

void WaitEnter(_team *pmyteam,_team *popteam,_player *pplayer,_ball *pball)//进入跑位状态，前往点位
{
		// setfillstyle(1,BLACK);
		// circle(10,10,10);
	if(pplayer->name==Player)
...
void WaitExecute(_team *pmyteam,_team *popteam,_player *pplayer,_ball *pball)//判断距离，进入行动状态
{
	if(pplayer->name==Player)
	{
		if(pmyteam->pnowstate==&pmyteam->Attack)
...
void ActioningEnter(_team *pmyteam,_team *popteam,_player *pplayer,_ball *pball)//进入行动状态，速度初始化为0
{
...
void ActioningExecute(_team *pmyteam,_team *popteam,_player *pplayer,_ball *pball)
{
	...
	if(pplayer->name==Player)
	{
		if(pmyteam->pnowstate==&pmyteam->Attack)
		{
			switch(pplayer->ID)
			{
				case(0):dy=abs(pplayer->now_pos.y-pball->now_pos.y);//纵坐标和球一致
						if(dy>=20.0)
						{
							pplayer->velocity.y=4.0*(pball->now_pos.y-pplayer->now_pos.y)/dy;
						}
						break;
				case(1):if((pball->now_pos.x-pplayer->now_pos.x-pplayer->velocity.x)>70)//x坐标相对与足球移动
							pplayer->velocity.x=4.0;
						else if((pball->now_pos.x-pplayer->now_pos.x-pplayer->velocity.x)<-70)
							pplayer->velocity.x=-4.0;
						else
							pplayer->velocity.x=0;
						// if(distance(pplayer->now_pos.x,pplayer->now_pos.y,pball->now_pos.x,pball->now_pos.y)<=50.0)//跑位，适当远离球
						// {
						// 	far_dir=get_dir(pplayer->now_pos,popteam->player[3].now_pos);
						// 	pplayer->velocity.x=-4.0*far_dir.x;
						// 	pplayer->velocity.y=-4.0*far_dir.y;
						// }
						// else if(distance(pplayer->now_pos.x,pplayer->now_pos.y,pball->now_pos.x,pball->now_pos.y)>=150.0)//跑位，适当靠近球
						// {
						...
						// }
						break;
				case(2):if((pball->now_pos.x-pplayer->now_pos.x-pplayer->velocity.x)>70)//x坐标相对与足球移动
							pplayer->velocity.x=4.0;
						else if((pball->now_pos.x-pplayer->now_pos.x-pplayer->velocity.x)<-70)
							pplayer->velocity.x=-4.0;
						else
							pplayer->velocity.x=0;
				case(3):if((pball->now_pos.x-pplayer->now_pos.x-pplayer->velocity.x)>70)//x坐标相对与足球移动
							pplayer->velocity.x=4.0;
						else if((pball->now_pos.x-pplayer->now_pos.x-pplayer->velocity.x)<-70)
							pplayer->velocity.x=-4.0;
						else
	...
	else
	{
		...
		if(popteam->pnowstate==&popteam->Defend)
		{
			switch(pplayer->ID)
			{
				case(0):if(distance(540.0,160.0,pball->now_pos.x,pball->now_pos.y)<100.0)//防守在一个圆内，圆内出现球就扑上去
						{
							near_dir=get_dir(pplayer->now_pos,pball->now_pos);
							pplayer->velocity.x=3*near_dir.x;
							pplayer->velocity.y=3*near_dir.y;
...
void Short_passEnter(_team *popteam,_team *pmyteam,_ball *pball)//popteam为控球球队
{
	Pos2d pass_dir;
	pball->control=-1;
...
void BallUpdate(_team *popteam,_team *pmyteam,_ball *pball)//pteam1为控球球队
{
	...
	if(pball->time>=100&&pmyteam->position==Left)
	{
		pball->flag=1;
	}
	if(pball->time>=45&&pmyteam->position==Right)
	{
		pball->flag=1;
	}
