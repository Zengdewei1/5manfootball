	...
	if (pmyteam->name == Player)
	{
		if (pmyteam->pnowstate = &pmyteam->Attack)
		{
			...
			case (1):
				...
				else if (distance(pplayer->now_pos.x, pplayer->now_pos.y, popteam->player[3].now_pos.x, popteam->player[3].now_pos.y) <= 100.0) //???????????????????????????????·????°??????3
				{
			...
			case (2):
				...
				else if (distance(pplayer->now_pos.x, pplayer->now_pos.y, pmyteam->player[3].now_pos.x, pmyteam->player[3].now_pos.y) <= 100.0) //??????????????????????????????????·????°??????
				{
			...
			case (3):
				...
				else if (distance(pplayer->now_pos.x, pplayer->now_pos.y, pmyteam->player[2].now_pos.x, pmyteam->player[2].now_pos.y) <= 100.0) //??????????????????????????????????·????°??????
				{
...
void xiuxi()
{
	...
	setfillstyle(1,GREEN);
	bar(0,0,640,480);
	puthz(240,50,"中场休息",32,32,BLUE);
	puthz(120,120,"两只球队交换左右场地",32,32,BLUE);
	puthz(160,200,"按键",32,32,BLUE);
	outtextxy(240,200,"Enter");
	puthz(320,200,"进入下半场",32,32,BLUE);
}
